...
class PanopticModel(pl.LightningModule):
    ...
    def forward(self, image):
        seg_output, depth_output, small_depth_output, pose_output = self.model(image)
    ...
    def optimizer_step(
        ...
        super().optimizer_step(
            epoch_nb, batch_nb, optimizer, optimizer_i, second_order_closure
        )
        if batch_nb == 0:
    ...
    def training_step(self, batch, batch_idx):
        ...
        seg_output, depth_output, small_depth_output, pose_outputs = self.forward(image)
        log = {}
        prefix = "train"

        loss = depth_output.compute_loss(
            copy.deepcopy(depth_target), log, f"{prefix}_detailed/loss/refined_disp"
        )
        if self.hparams.frozen_stereo_checkpoint is None:
            loss = loss + small_depth_output.compute_loss(
                depth_target, log, f"{prefix}_detailed/loss/train_cost_volume_disp"
            )
        loss = loss + seg_output.compute_loss(
            seg_target, log, f"{prefix}_detailed/loss/seg"
        )
        if pose_targets[0] is not None:
            loss = loss + pose_outputs.compute_loss(
                pose_targets, log, f"{prefix}_detailed/pose"
            )
        ...
        if (batch_idx % 200) == 0:
            with torch.no_grad():
                ...
                small_depth_vis = small_depth_output.get_visualization_img(
                    np.copy(left_image_np)
                )
                llog[f"{prefix}/small_disparity"] = wandb.Image(
                    small_depth_vis, caption=prefix
                )
                logger.log(llog)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            loss = loss + seg_output.compute_loss(
                seg_target, log, f"{prefix_loss}_detailed/loss/seg"
            )
            if pose_targets[0] is not None:
                loss = loss + pose_outputs.compute_loss(
                    pose_targets, log, f"{prefix_loss}_detailed/pose"
                )
    ...
    def val_dataloader(self):
        return common.get_loader(
            self.hparams, "val", preprocess_func=self.preprocess_func
        )
