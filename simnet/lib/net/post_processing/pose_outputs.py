...
class PoseOutput:
    ...
    def convert_to_numpy_from_torch(self):
        ...
        self.z_centroid_field = np.ascontiguousarray(
            self.z_centroid_field.cpu().numpy()
        )
...
def extract_peaks_from_centroid_nms(
    centroid_heatmap, min_distance=5, min_confidence=0.3
):
...
def extract_peaks_from_centroid_sorted(
    centroid_heatmap, min_confidence=0.15, min_distance=10
):
    peaks = peak_local_max(
        centroid_heatmap, min_distance=min_distance, threshold_abs=min_confidence
    )
...
def extract_latent_emb_from_peaks(
    heatmap_output, peaks, latent_emb_output, scale_factor=8
):
...
def draw_peaks(centroid_target, peaks):
    ...
    for ii in range(peaks.shape[0]):
        ...
        heatmap_img = cv2.putText(
            heatmap_img, str(ii), point, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2
        )
